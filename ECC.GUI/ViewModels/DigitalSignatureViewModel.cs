...
{
    internal class DigitalSignatureViewModel : BaseViewModel
    {
        ...
        public string? Message
        {
            ...
            set
            {
                VisibilityHint = value?.Length == 0 ? Visibility.Visible : Visibility.Hidden;
                SetProperty(ref _message, value);
            }
        }

        private Visibility _visibilityHint = Visibility.Visible;
        ...
        private void OnStartProtocolCommandExecuted(object? parameter)
        {
            ...
            if (_pointG.IsOnCurve && _pointG.Order >= ECurve.Prime / 3 && ECurve.Prime != ECurve.Order && MillerRabinPrimalityTest.IsPrimeNumber(_pointG.Order))
            {
                ...
                Task.Run(WriteSteps);
                ECDSA.Execute(Message ?? "", _pointG, SelectedDelay.Value);
            }
        }
